...
const AboutUs = () => {
    ...
    const stats = [
        ...
        { number: '500+', label: 'Happy Clients', color: 'from-pink-400 to-rose-500' },
        { number: '1000+', label: 'Properties Sold', color: 'from-blue-400 to-cyan-500' },
        { number: '50+', label: 'Expert Agents', color: 'from-purple-400 to-indigo-500' }
    ];
        ...
        <section ref={sectionRef} className='py-20 px-4 md:px-8 lg:px-12 bg-linear-to-br from-indigo-50 via-white to-purple-50 overflow-hidden relative'>
                ...
                <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
                    {values.map((value, index) => (
                        <div
                            ...
                            className={`bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transform transition-all duration-500 cursor-pointer border-2 relative ${activeCard === index ? 'scale-105 border-transparent' : 'border-gray-100'
                                }`}
                            style={{ transitionDelay: `${index * 100}ms` }}
                        >
                            <div>
            ...
            <div className={`max-w-4xl mx-auto text-center transition-all duration-1000 delay-800 relative z-10 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                <div className='bg-linear-to-r from-purple-600 to-blue-600 rounded-3xl p-8 md:p-12 shadow-2xl transform hover:scale-105 transition-all duration-300'>
                    ...
                    <button className='px-10 py-4 bg-white text-purple-600 font-bold rounded-xl shadow-lg hover:shadow-2xl transform hover:scale-110 active:scale-95 transition-all duration-200'>
                        Let's Get Started
                    </button>
                </div>
            </div>
